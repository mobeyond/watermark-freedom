...
export class BrowserProvider {
  async chat(params: BrowserChatParams) {
  ...
  private async chatWithChatGPT(params: { messages: ChatMessage[]; stream: boolean }) {
  ...
  private async streamAssistantText(page: Page) {
    ...
    async function* generator() {
      ...
      while (true) {
        ...
        if (current && current.length > last.length) {
          ...
          last = current;
          yield { choices: [{ delta: { content: delta } }] } as any;
          stableIterations = 0;
        } else {
          stableIterations += 1;
        }
  ...
  private async waitForAssistantFinalText(page: Page): Promise<string> {
    let last = "";
    let stableIterations = 0;
    for (let i = 0; i < 240; i++) {
      const current = await BrowserProvider.readLastAssistantText(page);
  ...
  private static async readLastAssistantText(page: Page): Promise<string> {
    return page.evaluate(() => {
      function getAssistantText(): string {
        // Try various selectors for assistant messages
        const assistantRoles = Array.from(
          document.querySelectorAll('[data-message-author-role="assistant"], [data-testid^="conversation-turn-"]')
        );
